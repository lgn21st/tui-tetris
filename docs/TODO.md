...
NOTE (2026-02-02): 本文件包含大量历史任务条目；当前状态以:
...
- [x] **2.2.1 创建 IncrementalRenderer**
  - 实现位置: `src/term/*` (framebuffer + diff flush)
  - 实现:
    ```rust
...
- [x] **2.2.2 集成到主循环**
  - 文件: `src/main.rs`
  - 替换: 旧 UI 全量渲染 → framebuffer diff flush
  - 验证: 视觉输出完全一致
